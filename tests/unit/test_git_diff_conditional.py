...
from scripts.generate_pipeline import GitDiffConditional, load_dynamic_pipeline

# Fixtures
SUFFIX = "PIPELINE_FILE"
...
def test_load_dynamic_pipeline_success(
        monkeypatch, mocker, logger
):
    monkeypatch.setenv(f"{PLUGIN_PREFIX}_{SUFFIX}", "pipeline.yml")
...
"""
    ...
    result = load_dynamic_pipeline(PLUGIN_PREFIX, SUFFIX)
    assert result == {"steps": [{"label": "test", "queue": "test"}]}

    open_mock.assert_called_once_with("pipeline.yml", "r")
    assert logger.record_tuples == [
        (LOGGER_NAME, 20, f"Checking env var: {PLUGIN_PREFIX}_{SUFFIX} for file name")
    ]


def test_load_dynamic_pipeline_file_not_found(
        monkeypatch, mocker, log_and_exit_mock, logger
):
    monkeypatch.setenv(f"{PLUGIN_PREFIX}_{SUFFIX}", "pipeline.yml")
    open_mock = mocker.patch(
...
def test_load_dynamic_pipeline_bad_yaml(
        monkeypatch, mocker, log_and_exit_mock, git_diff_conditional, logger
):
    monkeypatch.setenv(f"{PLUGIN_PREFIX}_{SUFFIX}", "pipeline.yml")
    open_mock = mocker.patch(
